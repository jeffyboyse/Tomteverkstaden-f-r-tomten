...
CREATE TABLE IF NOT EXISTS `Address` (
  ...
  `Stadnamn` varchar(50) NOT NULL,
  PRIMARY KEY (`AddressID`),
  KEY `FK_Address_Stad` (`Stadnamn`),
  CONSTRAINT `FK_Address_Stad` FOREIGN KEY (`Stadnamn`) REFERENCES `Stad` (`Stadnamn`)
...
-- Dumpar struktur för tabell Tomteverkstaden.Stad
DROP TABLE IF EXISTS `Stad`;
CREATE TABLE IF NOT EXISTS `Stad` (
  `Stadnamn` varchar(50) NOT NULL DEFAULT '',
  `Namn` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  PRIMARY KEY (`Stadnamn`) USING BTREE,
  KEY `FK_Stad_Land` (`Namn`),
  CONSTRAINT `FK_Stad_Land` FOREIGN KEY (`Namn`) REFERENCES `Land` (`Namn`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Dumpar data för tabell Tomteverkstaden.Stad: ~0 rows (ungefär)
